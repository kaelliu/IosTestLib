//
//  kCipher.cpp
//  cctest01
//
//  Created by kael on 12/4/12.
//
//

#include "kCipher.h"

const short kCipher::sendMap[] = {
    
};

const short kCipher::recvMap[] = {
    
};

kCipher::kCipher()
{
    m_cbSendRound = 0;
    m_cbRecvRound = 0;
    
    m_dwSendXorKey = 0;
    m_dwRecvXorKey = 0;
    
    m_dwSendPacketCount = 0;
}

int kCipher::seedRandMap(int wSeed){
    long dwHold = wSeed;
    long aaa = dwHold * 241103L + 2533101L;
    long bbb = 4294967296L;
    aaa %= bbb;
    return (int) (aaa >> 16);
}

unsigned char kCipher::mapRecvByte(int cbData)
{
    unsigned char cbMap;
    cbMap = (unsigned char) recvMap[cbData];
    return cbMap;
}

unsigned char kCipher::mapSendByte(int cbData){
    int len = cbData;
    if (len >= 256) {
        len = len % 256;
    }
    unsigned char cbMap;
    cbMap = (unsigned char) sendMap[len];
    return cbMap;
}

unsigned char* kCipher::decryptBuffer(unsigned char * buff, int len)
{
    for (int i = 0; i < len; i++) {
        if (buff[i] < 0) {
            buff[i] = mapRecvByte(256 + buff[i]);
        } else {
            buff[i] = mapRecvByte(buff[i]);
        }
    }
    return buff;
}

unsigned char* kCipher::encryptBuffer(unsigned char* buff, int len)
{
    for (int i = 0; i < len; i++) {
        if (buff[i] < 0) {
            buff[i] = mapSendByte(256 + buff[i]);
        } else {
            buff[i] = mapSendByte(buff[i]);
        }
    }
    return buff;
}